...
export class ExerciseForm extends Component {
  ...
  onSubmit = (data) => {
    if (data.exerciseName) {
      ...
      if (action === 'Adding') {
        return dispatch(addExercise(workoutId, data)).then(this.handleRes);
      }

      if (action === 'Editing') {
        return dispatch(editExercise(workoutId, exerciseId, data)).then(
          this.handleRes
        );
      }
    }
  ...
  render() {
    ...
    return (
      <div className="modal-backdrop">
        ...
        >
          {this.state.succuess ? (
            <SuccessStatus toggleForm={() => setEdit(false)} />
          ) : (
            <React.Fragment>
              <h3>
                {action}{' '}
